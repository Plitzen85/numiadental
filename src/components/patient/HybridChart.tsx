...
import { RefreshCcw, FileText, CheckCircle2, Activity, Stethoscope } from 'lucide-react';
import { useMarket } from '../../context/MarketContext';
import { ToothSVG, ToothSurface, ToothCondition, ToothSurfaceMap, CONDITION_COLORS, CONDITION_LABELS, defaultSurfaces } from './ToothSVG';
...
const TOOLBAR: { condition: ToothCondition; color: string; activeClass: string }[] = [
    { condition: 'healthy', color: '#ffffff', activeClass: 'bg-white text-black' },
    { condition: 'caries', color: '#ef4444', activeClass: 'bg-red-500/20 text-red-400 border border-red-500/40' },
    { condition: 'resin', color: '#3b82f6', activeClass: 'bg-blue-500/20 text-blue-400 border border-blue-500/40' },
    { condition: 'amalgam', color: '#8b5cf6', activeClass: 'bg-purple-500/20 text-purple-400 border border-purple-500/40' },
    { condition: 'crown', color: '#eab308', activeClass: 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/40' },
    { condition: 'extracted', color: '#000000', activeClass: 'bg-white/20 text-white border border-white/40' },
    { condition: 'implant', color: '#06b6d4', activeClass: 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/40' },
    { condition: 'root_canal', color: '#f97316', activeClass: 'bg-orange-500/20 text-orange-400 border border-orange-500/40' },
    { condition: 'veneer', color: '#ec4899', activeClass: 'bg-pink-500/20 text-pink-400 border border-pink-500/40' },
    { condition: 'fracture', color: '#6b7280', activeClass: 'bg-gray-500/20 text-gray-400 border border-gray-500/40' },
];
...
// ─── Component ───────────────────────────────────────────────────────────────
export const HybridChart: React.FC = () => {
    ...
    return (
        <div className="w-full flex gap-6 h-full min-h-[600px]">
            {/* ── LEFT: Quote Panel ─────────────────────────────────────────── */}
            <div className="w-72 flex-shrink-0 rounded-2xl p-5 flex flex-col gap-4 overflow-y-auto"
                        ...
                        {treatments.length === 0 ? (
                            ...
                            <div key={item.id} className="flex justify-between items-start text-xs border-b border-gray-50 pb-2">
                                <div className="flex gap-2 items-start">
                                    <div className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-[9px]"
                                        style={{ background: CONDITION_COLORS[item.condition] + '30', color: CONDITION_COLORS[item.condition] === 'transparent' ? '#3b82f6' : CONDITION_COLORS[item.condition] }}>
                                        {item.toothNumber}
                                    </div>
                                    <div>
            ...
            <div className="flex-1 glass-panel border border-white/10 rounded-2xl p-5 flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={reset} className="p-2 text-clinical/30 hover:text-red-400 transition-colors" title="Reiniciar todo">
                    ...
                    {chartMode === 'odontogram' && (
                        <motion.div key="odo" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex flex-col gap-4 overflow-auto flex-1">
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 bg-black/20 p-2 rounded-xl border border-white/5 self-center">
                                {TOOLBAR.map(({ condition, color, activeClass }) => (
                                    <button key={condition}
                                        ...
                                        <div className="w-2.5 h-2.5 rounded-sm flex-shrink-0" style={{ background: color === '#000000' ? '#6b7280' : color }} />
                                ...
                                {(() => {
                                    ...
                                    allTeeth.forEach(n => {
                                        ...
                                        (['buccal', 'lingual'] as const).forEach(side => {
                                            pd.probingDepth[side].forEach(v => { if (v >= 4) deepSites++; });
                                            pd.bleeding[side].forEach(v => { if (v) bleedingSites++; });
                                            pd.plaque[side].forEach(v => { if (v) plaqueSites++; });
                                        });
                                    });
                                    return (
                                        <>
                                            ...
                                            <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl px-4 py-2 text-center">
                                                <p className="text-orange-400 font-bold text-lg">{bleedingSites}</p>
                                                <p className="text-[10px] text-orange-400/70 uppercase font-bold">Sangrado BOP</p>
                                            </div>
                                            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-2 text-center">
                                                <p className="text-yellow-400 font-bold text-lg">{plaqueSites}</p>
                                                <p className="text-[10px] text-yellow-400/70 uppercase font-bold">Placa +</p>
                                            </div>
                                        </>
                                    );
                                })()}
